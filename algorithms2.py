...
class Algorithm(torch.nn.Module):
    ...
    def __init__(self, model, input_shape, train_givenY, hparams, zeroshot_init=None):
        ...
        self.zeroshot_init = zeroshot_init  # 新增：zero-shot初始化
...
class PRODEN(Algorithm):
    ...
    def __init__(self, model, input_shape, train_givenY, hparams, zeroshot_init=None):
        super(PRODEN, self).__init__(model, input_shape, train_givenY, hparams, zeroshot_init)
...
class CAVL(Algorithm):
    ...
    """
    def __init__(self, model, input_shape, train_givenY, hparams, zeroshot_init=None):
        super(CAVL, self).__init__(model, input_shape, train_givenY, hparams, zeroshot_init)
        self.network = model
        self.optimizer = torch.optim.Adam(
            self.network.parameters(),
            lr=self.hparams["lr"],
        ...
        if zeroshot_init is not None:
            ...
            combined_confidence = zeroshot_init * train_givenY  # 只保留partial label范围内的预测
            combined_confidence = combined_confidence / combined_confidence.sum(dim=1, keepdim=True)
            self.label_confidence = combined_confidence.double()
        else:
...
class CRDPLL(Algorithm):
    ...
    """
    def __init__(self, model, input_shape, train_givenY, hparams, zeroshot_init=None):
        super(CRDPLL, self).__init__(model, input_shape, train_givenY, hparams, zeroshot_init)
        self.network = model
        self.optimizer = torch.optim.Adam(
            self.network.parameters(),
            lr=self.hparams["lr"],
...
class CC(Algorithm):
    ...
    def __init__(self, model, input_shape, train_givenY, hparams, zeroshot_init=None):
        super(CC, self).__init__(model, input_shape, train_givenY, hparams, zeroshot_init)
