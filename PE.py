...
class Trainer:
    ...
    @torch.no_grad()
    def save_zeroshot_confidence(self):
        """保存zero-shot置信度用于后续初始化"""
        cfg = self.cfg
        ...
        zeroshot_confidence = torch.cat(zeroshot_predictions, dim=0)
        
        # 保存到文件
        save_dir = "zeroshot_confidence"
        os.makedirs(save_dir, exist_ok=True)
        save_path = os.path.join(save_dir, f"{cfg.dataset}_zeroshot_confidence.pth")
        torch.save(zeroshot_confidence, save_path)
        print(f"Zero-shot confidence saved to {save_path}")
    ...
    def build_criterion(self):
        ...
        # 加载zero-shot置信度
        zeroshot_init = None
        if hasattr(cfg, 'init_with_zeroshot') and cfg.init_with_zeroshot:
            ...
            if os.path.exists(zeroshot_path):
                ...
                # 确保大小匹配
                if zeroshot_confidence.shape[0] >= N:
                ...
                print(f"Zero-shot confidence not found at {zeroshot_path}, using original initialization")
        
        algorithm_class = get_algorithm_class(cfg["loss_type"])
        self.algorithm = algorithm_class(self.model, tensor.shape, self.partialY, cfg, zeroshot_init=zeroshot_init)
